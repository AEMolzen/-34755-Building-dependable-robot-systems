...
def drive_forward(target_distance_meters):
    ...
    initial_x = pose.pose[0]
    initial_y = pose.pose[1]
    initial_distance = math.sqrt(initial_x**2 + initial_y**2)
    current_distance = initial_distance

    # PI Controller Gains for Driving (Tune these!)
    Kp_drive = 0.8  # Proportional gain for driving
    Ki_drive = 0.1  # Integral gain for driving

    integral_error = 0.0
    last_time = t.time()

    while abs(current_distance - initial_distance - target_distance_meters) > 0.01: # 1cm tolerance
        current_x = pose.pose[0]
        current_y = pose.pose[1]
        ...
        # Calculate error
        error = target_distance_meters - (current_distance - initial_distance)

        # Accumulate integral error
        integral_error += error * dt

        # Calculate control output (forward speed)
        speed = Kp_drive * error + Ki_drive * integral_error

        # Limit speed
        max_speed = 0.5  # m/s (Adjust as needed)
        speed = max(min(speed, max_speed), -max_speed)

        # Send drive command
        service.send(service.topicCmd + "ti/rc", f"{speed} 0")
        #print(f"Current Distance: {current_distance:.3f}, Target: {initial_distance + target_distance_meters:.3f}, Error: {error:.3f}, Speed: {speed:.3f}")
        t.sleep(0.01)
...
def turn(target_angle_rad):
    ...
    Kp_turn = 3.0  # Proportional gain for turning
